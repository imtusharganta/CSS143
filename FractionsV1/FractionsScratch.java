...
public class FractionsScratch {
  ...
  /**
   * contrasting the isUnique, this method is used to check if the number
   * 
   * @param numerator
   * @param denominator
   * @return
   */
  public int repeat(int numerator, int denominator) {
    if (denominator == 0) {
      System.out.println("Can't divide by 0");
    }
    int repetition = 0;
    for (int i = 0; i <= fracArray.length; i += 2) {
      int num = fracArray[i];
      int den = fracArray[i + 1];
      if (crossMultiplyEquals(numerator, denominator, num, den))
        repetition++;
    }
    return repetition;
  }
