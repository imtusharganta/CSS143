...
public class Money implements Comparable, Cloneable, Serializable {
  ...
  public boolean equals(Object o) {
    ...
    if (copy.getMoney() == this.getMoney()) {
      doesEqual = true;
    } else {
      doesEqual = false;
    }
  ...
  /**
   * This is the compareTo method which we will override
   * We will first check if the object is an instance of money
   * if it is, if they equal, we return 0, if our current object is
   * greater, we return, else we return -1 because the object we passed in
   * was greater. If it is not a money object, we throw an exception.
   */
  @Override
  public int compareTo(Object obj) throws IllegalArgumentException {
    if (obj instanceof Money) {
      ...
      if (copy.equals(this)) {
        return 0;
      } else if (copy.getMoney() < this.getMoney()) {
        return 1;
      } else {
        return -1;
      }
  ...
  /**
   * This is the clone method we are overriding from the Cloneable interface.
   * Makes a clone and returns the clone.
   */
  @Override
