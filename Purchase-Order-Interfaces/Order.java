...
public class Order implements Comparable, Cloneable, Serializable {
   ...
   * @throws CloneNotSupportedException
   */
  public Date getOrderDate() throws CloneNotSupportedException {
    // return new Date(orderDate);
    return orderDate.clone();
  }
   ...
   * @return
   */
  public boolean setFulfilled(Date sentDate) throws CloneNotSupportedException {
    if (orderDate.isAfter(sentDate) && !isFulfilled()) {
      this.sentDate = sentDate.clone();
      return true;
    }
    return false;
  }
   ...
   * @return
   */
  public boolean setOrderDate(Date nextDate) throws CloneNotSupportedException {
    if (isFulfilled()) {
      return false;
    } else {
      this.orderDate = nextDate.clone();
   ...
   * @return
   */
  public boolean setAmount(Money amount) throws CloneNotSupportedException {
    if (isFulfilled()) {
      return false;
    } else {
  ...
  public boolean equals(Object toCompare) {
    Order copy = (Order) toCompare;
    boolean doesEqual = false;
    if (this.customer.equalsIgnoreCase(copy.customer) && this.item.equalsIgnoreCase(copy.item)) {
      doesEqual = true;
    } else {
      doesEqual = false;
    }
  ...
  /**
   * This is the compareTo method for Order
   * If the object is an instace of Order
   * we first cast the object to an order and set our Money to null, and we
   * will try to set our money opject to the objects amount
   * We will then compare the current objects amount to the amount we just copied
   * if we cant, then we will catch an Error and then return -1.
   */
  public int compareTo(Object obj) throws IllegalArgumentException {
    if (obj instanceof Order) {
      Order copy = (Order) obj;
      Money amount = null;
      try {
        amount = copy.getAmount();
        return this.getAmount().compareTo(amount);
      } catch (Exception e) {
  ...
  /**
   * This is the clone method for the order object
   * we will first clone our order.
   * And if the order is fulfilled, then copy everything to our copy
   * else, we copy everything besides the sentDate, and return the copy.
   */
