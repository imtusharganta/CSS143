...
public class Date implements Comparable, Cloneable, Serializable {
  ...
  /**
   * This is the compareTo method which we are overriding
   * from the comparable interface.
   * We will check if the object passed in is a Date, and it will
   * return a value from -1, 0, or 1 by checking if the object passed in
   * is greater, equal to, or less than the current object we are comparing to.
   * if the object is not an instance of date, we will throw an exception.
   */
  @Override
  ...
  /**
   * This overrides the clone method from the cloneable interface.
   * we will clone it and return the cloned object.
   */
  @Override
